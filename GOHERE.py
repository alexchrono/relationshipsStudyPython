...
#  in.
#this tests the many-to-many relationship between students and courses.
...
# def studentsTaughtByTeacher(teacher_name):
#     session = db.session
#     teacher = aliased(Teacher)
#     student = aliased(Student)

#     query = session.query(student.name).select_from(teacher).join(teacher.students).join(student).filter(teacher.name == teacher_name)
#     students_taught_by_teacher = query.all()

#     if students_taught_by_teacher:
#         return students_taught_by_teacher
#     else:
#         return f"No students found for teacher: {teacher_name}"

# with app.app_context():
#     teacher_name_to_query = random.choice(all_teachers)[0]
#     students_taught_by_teacher = studentsTaughtByTeacher(teacher_name_to_query)
#     print(' ')
#     print(' ')
#     print('*************#3  students taught by teacher**************')
#     print('many-to-many relationship between teachers and students is operational')
#     print(f"Students taught by {teacher_name_to_query}: {students_taught_by_teacher}")
